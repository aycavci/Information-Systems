...
class KDTree:
    ...
    def __init__(self, db, options):
        ...
        self.storage = [{} for x in range(storage_size)]

        self.traverse(data)

        self.bb = bb.BoundingBox.from_dataset(
            data, db.fields()["x"], db.fields()["y"])
    ...
    def traverse(self, mtrx, depth=0, sidx=si.StorageIndex()):
        ...
        if len(mtrx) == 1 or depth + 1 == self.max_depth:
            ...
            self.storage[sidx.storage()]["partition"] = mtrx[order[0]
                                                             [-1], axis + 1]
    ...
    def traverse_partition(self, depth, sidx, bboxes, box):
        ...
        if "index" in self.storage[sidx.storage()]:
            ...
            if "partition" in self.storage[sidx.storage()]:
                ...
                left_box = box.reduce_max(
                    axis, self.storage[sidx.storage()]["partition"])
                right_box = box.reduce_min(
                    axis, self.storage[sidx.storage()]["partition"])

                self.traverse_partition(
                    depth + 1, sidx.left(), bboxes, left_box)
                self.traverse_partition(
                    depth + 1, sidx.right(), bboxes, right_box)
